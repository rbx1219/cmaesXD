...
// Constructors
F23_noncontinuous_rotated_hybrid_composition_3::F23_noncontinuous_rotated_hybrid_composition_3 (int dimension, double bias) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME23;
	...
	loadMatrixFromFile(DEFAULT_FILE_DATA23, NUM_FUNC, m_dimension, m_o);
	// Load the matrix
	char tmp[4];
	sprintf(tmp,"%d",m_dimension);
	loadNMatrixFromFile(DEFAULT_FILE_MX_PREFIX23 + tmp + DEFAULT_FILE_MX_SUFFIX23, NUM_FUNC, m_dimension, m_dimension, m_M);

	// Initialize the hybrid composition job object
	theJob.num_func = NUM_FUNC;
	theJob.num_dim = m_dimension;
	theJob.C = 2000.0;
	theJob.sigma = m_sigma23;
	theJob.biases = m_func_biases23;
	theJob.lambda = m_lambda23;
	theJob.o = m_o;
	theJob.M = m_M;
	theJob.w = m_w;
	theJob.z = m_z;
	...
	for (int i = 0 ; i < NUM_FUNC ; i ++) {
		for (int j = 0 ; j < m_dimension ; j ++) {
			m_testPoint[j] = (5.0 / m_lambda23[i]);
		}
...
F23_noncontinuous_rotated_hybrid_composition_3::F23_noncontinuous_rotated_hybrid_composition_3 (int dimension, double bias, string file_data, string file_m) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME23;
...
double MyHCJob23::basic_func(int func_no, double *x,int length) {
	double result = 0.0;
	switch(func_no) {
		case 0:
		case 1:
...
// Function body
double F23_noncontinuous_rotated_hybrid_composition_3::f(double *x,int length) {
