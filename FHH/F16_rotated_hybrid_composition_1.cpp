...
double MyHCJob16::basic_func(int func_no, double *x,int length) {
	double result = 0.0;
	switch(func_no) {
		case 0:
		case 1:
...
// Constructors
F16_rotated_hybrid_composition_1::F16_rotated_hybrid_composition_1 (int dimension, double bias) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME16;
	...
	loadMatrixFromFile(DEFAULT_FILE_DATA16, NUM_FUNC, m_dimension, m_o);
	// Load the matrix
	char tmp[4];
	sprintf(tmp,"%d",m_dimension);
	loadNMatrixFromFile(DEFAULT_FILE_MX_PREFIX16 + tmp + DEFAULT_FILE_MX_SUFFIX16, NUM_FUNC, m_dimension, m_dimension, m_M);

	// Initialize the hybrid composition job object
	theJob.num_func = NUM_FUNC;
	theJob.num_dim = m_dimension;
	theJob.C = 2000.0;
	theJob.sigma = m_sigma16;
	theJob.biases = m_func_biases16;
	theJob.lambda = m_lambda16;
	theJob.o = m_o;
	theJob.M = m_M;
	theJob.w = m_w;
	theJob.z = m_z;
	...
	for (int i = 0 ; i < NUM_FUNC ; i ++) {
		for (int j = 0 ; j < m_dimension ; j ++) {
			m_testPoint[j] = (5.0 / m_lambda16[i]);
		}
...
F16_rotated_hybrid_composition_1::F16_rotated_hybrid_composition_1 (int dimension, double bias, string file_data, string file_m) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME16;
...
// Function body
double F16_rotated_hybrid_composition_1::f(double *x,int length) {
