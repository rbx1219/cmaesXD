...
// Constructors
F20_rotated_hybrid_composition_2_global_opt_bound::F20_rotated_hybrid_composition_2_global_opt_bound (int dimension, double bias) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME20;
	...
	loadMatrixFromFile(DEFAULT_FILE_DATA20, NUM_FUNC, m_dimension, m_o);
	...
	loadNMatrixFromFile(DEFAULT_FILE_MX_PREFIX20 + tmp + DEFAULT_FILE_MX_SUFFIX20, NUM_FUNC, m_dimension, m_dimension, m_M);
	// Initialize the hybrid composition job object
	theJob.num_func = NUM_FUNC;
	theJob.num_dim = m_dimension;
	theJob.C = 2000.0;
	theJob.sigma = m_sigma20;
	theJob.biases = m_func_biases20;
	theJob.lambda = m_lambda20;
	theJob.o = m_o;
	theJob.M = m_M;
	theJob.w = m_w;
	theJob.z = m_z;
	...
	for (int i = 0 ; i < NUM_FUNC ; i ++) {
		for (int j = 0 ; j < m_dimension ; j ++) {
			m_testPoint[j] = (5.0 / m_lambda20[i]);
		}
...
F20_rotated_hybrid_composition_2_global_opt_bound::F20_rotated_hybrid_composition_2_global_opt_bound (int dimension, double bias, string file_data, string file_m) {
	m_dimension = dimension;
	m_bias = bias;
	m_func_name = FUNCTION_NAME20;
...
double MyHCJob20::basic_func(int func_no, double *x,int length) {
	double result = 0.0;
	switch(func_no) {
		case 0:
		case 1:
...
// Function body
double F20_rotated_hybrid_composition_2_global_opt_bound::f(double *x,int length) {
