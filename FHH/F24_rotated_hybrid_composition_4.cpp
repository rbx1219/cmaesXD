...
// Constructors
F24_rotated_hybrid_composition_4::F24_rotated_hybrid_composition_4 (int dimension, double bias) {
	...
	loadMatrixFromFile(DEFAULT_FILE_DATA24, NUM_FUNC, m_dimension, m_o);
	// Load the matrix
	char tmp[4];
	sprintf(tmp,"%d",m_dimension);
	loadNMatrixFromFile(DEFAULT_FILE_MX_PREFIX24 + tmp + DEFAULT_FILE_MX_SUFFIX24, NUM_FUNC, m_dimension, m_dimension, m_M);
...
// Function body
double F24_rotated_hybrid_composition_4::f(double *x,int length) {
